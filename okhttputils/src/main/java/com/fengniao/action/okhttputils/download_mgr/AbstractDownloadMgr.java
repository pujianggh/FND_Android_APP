package com.fengniao.action.okhttputils.download_mgr;
...
public abstract class AbstractDownloadMgr {
    ...
    private com.fengniao.action.okhttputils.OkHttpCall mOkHttpCall;
    ...
    public AbstractDownloadMgr(Builder builder) {
        mOkHttpCall = builder.mOkHttpCall;
        ...
        mDownloadTaskListener = new DownloadTaskListener() {
            ...
            public void onStart(String taskId, long completeBytes, long totalBytes) {
                if(DEBUG) {
                    LogOkHttpUtils.d(TAG, "onStart " + taskId + " startCompleteBytes=" + completeBytes + " totalBytes=" + totalBytes);
                }
            ...
            public void onProgress(String taskId, long currentBytes, long totalBytes) {
                if(DEBUG) {
                    LogOkHttpUtils.d(TAG, "onProgress " + taskId + " currentBytes=" + currentBytes + " totalBytes=" + totalBytes);
                }

                if(mDownloadTaskPool.get(taskId).getNextSaveBytes() > mSaveProgressBytes) {     //每mSaveProgressBytes保存一次进度
                    if(DEBUG) {
                        LogOkHttpUtils.d(TAG, "saveProgress");
                    }
            ...
            public void onPause(String taskId, long currentBytes, long totalBytes) {
                if(DEBUG) {
                    LogOkHttpUtils.d(TAG, "onPause " + taskId + " currentBytes=" + currentBytes + " totalBytes=" + totalBytes);
                    LogOkHttpUtils.d(TAG, "saveProgress");
                }
            ...
            public void onFinish(String taskId, File file) {
                if(DEBUG) {
                    LogOkHttpUtils.d(TAG, "onFinish " + taskId + " filePath=" + file.getAbsolutePath());
                }
            ...
            public void onFailure(String taskId, String error_msg) {
                if(DEBUG) {
                    LogOkHttpUtils.d(TAG, "onFailure " + taskId + " " + error_msg);
                }
    ...
    public DownloadTask addTask(Task task) {
        if(DEBUG) {
            LogOkHttpUtils.d(TAG, "addTask " + task.toString());
        }
        ...
        if(mDownloadTaskPool.containsKey(task.getTaskId())) {       //task已经加过
            if(DEBUG) {
                LogOkHttpUtils.d(TAG, "addTask contain " + task.getTaskId());
            }
            return null;
        }
        ...
        downloadTask.setMyOkHttp(mOkHttpCall);
    ...
    public void startTask(String taskId) {
        ...
        if(DEBUG) {
            LogOkHttpUtils.d(TAG, "startTask " + taskId);
        }
    ...
    public void startAllTask() {
        if(DEBUG) {
            LogOkHttpUtils.d(TAG, "startAllTask");
        }
    ...
    public void pauseTask(String taskId) {
        ...
        if(DEBUG) {
            LogOkHttpUtils.d(TAG, "pauseTask " + taskId);
        }
    ...
    private void startNextTask() {
        ...
        for(int i = 0; i < mDownloadTaskQuene.size(); i ++) {
            ...
            if(downloadTask.getStatus() == DownloadStatus.STATUS_WAIT) {
                if(DEBUG) {
                    LogOkHttpUtils.d(TAG, "startNextTask " + downloadTask.getTaskId());
                }
    ...
    public static abstract class Builder {
        ...
        public Builder() {
            mOkHttpCall = new com.fengniao.action.okhttputils.OkHttpCall();
            mMaxDownloadIngNum = DEFAULT_MAX_DOWNLOADING_NUM;
            mSaveProgressBytes = DEFAULT_SAVE_PROGRESS_BYTES;
        }

        public Builder myOkHttp(@NonNull com.fengniao.action.okhttputils.OkHttpCall myOkHttp) {
            mOkHttpCall = myOkHttp;
            return this;
        }
